...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread, struct task_struct* parent_thread) {
	...
	block_desc_init(child_thread->u_block_desc); // 初始化块描述符, 共用一个会导致缺页异常

	/* b 复制父进程的虚拟地址池的位图 */
	uint32_t bitmap_pg_cnt = DIV_ROUND_UP((0xc0000000 - USER_VADDR_START) / PG_SIZE / 8 , PG_SIZE);
	void* vaddr_btmp = get_kernel_pages(bitmap_pg_cnt); // 新分配一块位图空间
...
static void copy_body_stack3(struct task_struct* child_thread, struct task_struct* parent_thread, void* buf_page) {
	...
	/* 在父进程的用户空间中查找已有数据的页 */
	while (idx_byte < btmp_bytes_len) { // 按字节查看虚拟地址位图
		if (vaddr_btmp[idx_byte]) { // 如果分配了内存
			idx_bit = 0;
			while (idx_bit < 8) { // 按位查看该字节内容
				if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte]) { // 如果该位为1
