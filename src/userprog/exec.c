...
static bool segment_load(int32_t fd, uint32_t offset, uint32_t filesz, uint32_t vaddr) {
	...
	uint32_t occupy_pages = 0; // 该段占用的总页框数
	...
	/* 3. 从文件开始处读取filesz字节的内容到虚拟vaddr缓冲区处 */
...
static int32_t load(const char* pathname) {
	...
	/* 校验elf头 */
	if (memcmp(elf_header.e_ident, "\177ELF\1\1\1", 7) /* 文件魔数 */ \
		|| elf_header.e_type != 2 /* 文件类型ET_EXEC */ \
		|| elf_header.e_machine != 3 /* 体系结构EM_386 */ \
		|| elf_header.e_version != 1 /* 版本信息1 */ \
		|| elf_header.e_phnum > 1024 /* 程序头表条目的数量段的个数 */ \
		|| elf_header.e_phentsize != sizeof(struct Elf32_Phdr)) /* 程序头表每个条目的字节大小 */ {
		ret = -1;
		goto done;
	}
...
int32_t sys_execv(const char* path, const char* argv[]) {
	...
	/* 参数传递给用户进程 */
	intr_0_stack->ebx = (int32_t)argv; // 参数地址
	intr_0_stack->ecx = argc; // 参数个数
	intr_0_stack->eip = (void*)entry_point; // 将可执行文件入口地址赋值
	/* 使新用户进程的栈地址为最高用户空间地址 */
