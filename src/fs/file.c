...
// 在目录parent_dir中以模式flag创建普通文件filename, 成功则返回文件描述符, 即pcb->fd_table的下标
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    ...
    /* 1. 判断文件是否是第一次写, 如果是, 先为其分配一个块, 同步到硬盘
	与目录项不同, 文件写入是从0开始, 目录项是单独的个体, 可以处于不同的
	块中 */
	...
	/* 2. 开始将所有块地址收集到 all_blocks (系统中块大小等于扇区大小)
    ...
    while (bytes_written < count) {
        ...
        sec_lba = all_blocks[sec_idx]; // 获得已经使用的扇区地址
        sec_off_bytes = file->fd_inode->i_size % BLOCK_SIZE; // 最后一个扇区内的结尾偏移地址
        sec_left_bytes = BLOCK_SIZE - sec_off_bytes; // 最后一个扇区剩余可写入的数据大小
 
        // 判断此次写入硬盘的数据大小, 如果剩余size比可写入数据小, 则使用剩余size
        chunk_size = size_left < sec_left_bytes ? size_left : sec_left_bytes;
        if (first_write_block) { // 第一次写入扇区剩余容量中: 先读取, 后写入
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
	...
	/* 1. 若要读取的字节数超过了文件可读的剩余量, 就用剩余量作为待读取的字节数 */
    ...
    /* 2. 开始构建 all_blocks 块地址数组, 专门存储用到的块地址 */
    if (read_blocks == 0) {
    ...
    /* 3. 用到的块地址已经收集到 all_blocks 中, 下面开始读数据 */
