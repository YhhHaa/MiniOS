...
bool search_dir_entry(struct partition* part, struct dir* pdir, const char* name, struct dir_entry* dir_e) {
	...
	// 将inode的i_sectors数组内存的扇区地址复制到all_blocks方便索引
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
    ...
    uint32_t dir_entry_size = cur_part->sb->dir_entry_size; // 目录项的大小记录在超级块中
 
    ASSERT(dir_size % dir_entry_size == 0);
 
    // 每扇区最大的目录项数目, 防止跨扇区写入目录项
    ...
    while (block_idx < 140) {
        block_bitmap_idx = -1;
        if (all_blocks[block_idx] == 0) { // 找到空位
            block_lba = block_bitmap_alloc(cur_part); // 空闲块位图申请可用1个扇区地址
            if (block_lba == -1) {
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
    ...
    while (block_idx < 140) {
        ...
        // b. 遍历所有的目录项, 统计该扇区的目录项数量及是否有待删除的目录项, 没有待删除的目录项则继续遍历下一个块
        while (dir_entry_idx < dir_entrys_per_sec) {
            if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
                if (!strcmp((dir_e + dir_entry_idx)->filename, ".")) { // 如果目录项名为 . ，判断为最初的块
                    is_dir_first_block = true;
                } else if (strcmp((dir_e + dir_entry_idx)->filename, ".") && // 不是.以及.., 统计个数
                           strcmp((dir_e + dir_entry_idx)->filename, "..")) {
                    dir_entry_cnt++; // 统计此扇区内的目录项个数, 用来判断删除目录项后是否回收该扇区
                    if ((dir_e + dir_entry_idx)->i_no == inode_no) { // 找到该目录项
        ...
        // c. 在此扇区中找到目录项后, 清除该目录项并判断是否回收扇区, 随后退出循环直接返回
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
    ...
    // 1. 在父目录 parent_dir 中删除子目录 child_dir 对应的目录项
    delete_dir_entry(cur_part, parent_dir, child_dir_inode->i_no, io_buf);
 
    // 2. 回收 inode 中 i_sectors 中所占用的扇区, 并同步 inode_bitmap 和 block_bitmap
